...
trait Parsers {
  ...
  private[combinator] def Success[U](res: U, next: Input, failure: Option[Failure]): ParseResult[U] =
    new Success(res, next) {
      override val lastFailure: Option[Failure] = failure
    }
  ...
  abstract class Parser[+T] extends (Input => ParseResult[T]) {
   ...
   def ~~ [U, V](q: => Parser[U])(implicit combine: (T, U) => V): Parser[V] = new Parser[V] {
    ...
    = OnceParser {
      (for (a <- this; b <- commit(p)) yield new~(a, b)).named("~!")
    }
    ...
    def ~>![U](q: => Parser[U]): Parser[U] = {
      lazy val p = q // lazy argument
      OnceParser {
        (for (_ <- this; b <- commit(p)) yield b).named("~>!")
      }
    }
    ...
    def <~![U](q: => Parser[U]): Parser[T] = {
      lazy val p = q // lazy argument
      OnceParser {
        (for (a <- this; _ <- commit(p)) yield a).named("<~!")
      }
    }
